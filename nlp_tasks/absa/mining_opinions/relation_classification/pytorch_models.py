...
class SpanBasedModelForRC(Model):
    def __init__(self, word_embedder: TextFieldEmbedder, position_embedder: TextFieldEmbedder,
                 polarities: list, vocab: Vocabulary, configuration: dict):
        super().__init__(vocab)
        self.configuration = configuration
        ...
        self.lstm = torch.nn.LSTM(lstm_input_size, int(word_embedding_dim / 2), batch_first=True,
                                  bidirectional=True, num_layers=num_layers, dropout=0.5)
        sentiment_fc_input_size = word_embedding_dim * 2
        self.sentiment_fc = nn.Sequential(nn.Linear(sentiment_fc_input_size, sentiment_fc_input_size),
                                          nn.ReLU(),
                                          nn.Linear(sentiment_fc_input_size, 1))
        self.dropout_after_embedding_layer = nn.Dropout(0.5)
        self.dropout_after_lstm_layer = nn.Dropout(0.5)

    def get_term_representation(self, tensor: torch.Tensor, start: int, end: int):
        """
        ...
        :return:
        """
        word_representations = tensor[start: end]
        term_word_num = end - start
        if term_word_num > 1:
            term_representation = torch.sum(word_representations, dim=0) / len(word_representations)
            term_representation = term_representation.unsqueeze(0)
        else:
            term_representation = word_representations
    ...
    def forward(self, tokens: Dict[str, torch.Tensor], label: torch.Tensor, position: torch.Tensor,
        ...
        representations = []
        for i, element in enumerate(sample):
            aspect_term = element['opinion']['aspect_term']
            aspect_term_start = aspect_term['start']
            aspect_term_end = aspect_term['end']
            aspect_term_representation = self.get_term_representation(lstm_result[i], aspect_term_start, aspect_term_end)

            opinion_term = element['opinion']['opinion_term']
            opinion_term_start = opinion_term['start']
            opinion_term_end = opinion_term['end']
            opinion_term_representation = self.get_term_representation(lstm_result[i], opinion_term_start, opinion_term_end)

            term_representation_cat = torch.cat([aspect_term_representation, opinion_term_representation], dim=1)
            representations.append(term_representation_cat)
        representations_cat = torch.cat(representations, dim=0)

        relation_output = self.sentiment_fc(representations_cat).squeeze(dim=1)

        output = {}
        if label is not None:
            loss = self.sentiment_loss(relation_output, label.float())
            if torch.isnan(loss):
                print()

            output['logit'] = relation_output
            output['label'] = label
            output['loss'] = loss
...
class SpanBasedModelForMultiRC(Model):
    def __init__(self, word_embedder: TextFieldEmbedder, position_embedder: TextFieldEmbedder,
                 polarities: list, vocab: Vocabulary, configuration: dict):
        super().__init__(vocab)
        self.configuration = configuration
        ...
        self.sentiment_fc = nn.Sequential(nn.Linear(sentiment_fc_input_size, sentiment_fc_input_size),
                                          nn.ReLU(),
                                          nn.Linear(sentiment_fc_input_size, len(self.configuration['relations'])))
        self.dropout_after_embedding_layer = nn.Dropout(0.5)
        self.dropout_after_lstm_layer = nn.Dropout(0.5)
    ...
    def forward(self, tokens: Dict[str, torch.Tensor], label: torch.Tensor, position: torch.Tensor,
        ...
        output = {}
        if label is not None:
            loss = self.sentiment_loss(relation_output, label.long())
            if torch.isnan(loss):
                print()
...
class SpanBasedModelForMultiRCBert(Model):
    def __init__(self, word_embedder: TextFieldEmbedder, position_embedder: TextFieldEmbedder,
    ...
    def forward(self, tokens: Dict[str, torch.Tensor], label: torch.Tensor, position: torch.Tensor,
                sample: list, bert: torch.Tensor=None) -> torch.Tensor:
        mask = get_text_field_mask(tokens)
        word_embeddings = self.word_embedder(tokens)

        word_embeddings_size = word_embeddings.size()
        word_embeddings = self.get_bert_embedding(bert, sample, word_embeddings_size)
...
class SpanBasedModelForRCEstimator(Estimator):
    def __init__(self, model: Model, iterator: DataIterator, polarities: list,
                 cuda_device: int = -1, configuration: dict=None) -> None:
        super().__init__()
        self.model = model
    ...
    def generate_f1_score(self, logits: torch.Tensor, labels: torch.Tensor):
        """
        ...
        pred = logits > 0.5
        pred = pred.int()
        return f1_score(labels.detach().numpy(), pred.detach().numpy())
    ...
    def estimate(self, ds: Iterable[Instance]) -> dict:
        ...
        with torch.no_grad():
            ...
            for batch in pred_generator_tqdm:
                ...
                logits.append(logit)
                labels.append(label)
            logit_total = torch.cat(logits, dim=0)
            label_total = torch.cat(labels, dim=0)
            f1 = self.generate_f1_score(logit_total, label_total)
        return {'rc_f1': f1}


class SpanBasedModelForMultiRCEstimator(Estimator):
    def __init__(self, model: Model, iterator: DataIterator, polarities: list,
                 cuda_device: int = -1, configuration: dict=None) -> None:
        super().__init__()
        self.model = model
    ...
    def generate_f1_score(self, logits: torch.Tensor, labels: torch.Tensor):
        ...
        f1 = f1_score(labels.detach().numpy(), pred.detach().numpy(), average='micro', labels=positive_labels)
        recall = recall_score(labels.detach().numpy(), pred.detach().numpy(), average='micro', labels=positive_labels)
        precision = precision_score(labels.detach().numpy(), pred.detach().numpy(), average='micro', labels=positive_labels)
    ...
    def estimate(self, ds: Iterable[Instance]) -> dict:
        ...
        with torch.no_grad():
            ...
            logit_total = torch.cat(logits, dim=0)
            label_total = torch.cat(labels, dim=0)
            result = self.generate_f1_score(logit_total, label_total)
            result['rc_f1'] = result['f1']
        return result
...
class SpanBasedModelForRCPredictor(predictor):
    def __init__(self, model: Model, iterator: DataIterator, polarities: list,
                 cuda_device: int = -1, configuration: dict=None) -> None:
        super().__init__()
        self.model = model
...
class SpanBasedModelForMultiRCPredictor(predictor):
    def __init__(self, model: Model, iterator: DataIterator, polarities: list,
                 cuda_device: int = -1, configuration: dict=None) -> None:
        super().__init__()
        self.model = model
    ...
    def convert_index_into_relation(self, indices: torch.Tensor):
        """
        ...
        relations = self.configuration['relations']
    ...
    def predict(self, ds: Iterable[Instance]) -> dict:
        ...
        with torch.no_grad():
            ...
            logit_total = torch.cat(logits, dim=0)
            pred_total = logit_total.argmax(dim=-1)
            pred_relations = self.convert_index_into_relation(pred_total)

            label_total = torch.cat(labels, dim=0)
            true_relations = self.convert_index_into_relation(label_total)
        result = {'samples': samples, 'pred': pred_relations, 'true': true_relations}
        return result