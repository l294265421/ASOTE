...
from nlp_tasks.absa.mining_opinions.relation_classification import relation_classification_train_templates as templates


parser = argparse.ArgumentParser()
parser.add_argument('--current_dataset', help='dataset name', default='RealASOTripletRest16', type=str)
parser.add_argument('--task_name', help='task name', default='relation_classification', type=str)
parser.add_argument('--data_type', help='task name', default='common', type=str)
parser.add_argument('--model_name', help='model name', default='LSTM-multi', type=str)
...
parser.add_argument('--relations', help='ate result filepath',
                    default=['other', 'positive', 'neutral', 'negative'], type=list)
...
# configuration['data_type'] = 'data_type.{data_type}-sample_mode.{sample_mode}-aspect_term_aware.{aspect_term_aware}-pair.{pair}'.format_map(configuration)
configuration['data_type'] = 'data_type.{data_type}'.format_map(configuration)
...
if model_name in ['LSTM']:
    template = templates.SpanBasedModelForRelationClassification(configuration_for_this_repeat)
if model_name in ['LSTM-multi']:
    template = templates.SpanBasedModelForMultiRelationClassification(configuration_for_this_repeat)
if model_name in ['bert-multi']:
    template = templates.SpanBasedModelForMultiRelationClassificationBert(configuration_for_this_repeat)
