...
parser.add_argument('--current_dataset', help='dataset name', default='ASOTEDataRest16', type=str)
parser.add_argument('--triplet_result_filepath', help='triplet result filepath',
                    default=os.path.join(common_path.project_dir, 'AGF-ASOTE-data', 'absa', 'ASOTE-baselines-prediction-result', 'MTL', 'mtl_results', 'asmote_rest_16_mtl_0%d.json'), type=str)
...
def get_sentence_and_triplets_pred(triplets_pred):
    ...
    for e in triplets_pred:
        ...
        for opinion in pred:
            triplet_num_pred += 1
            opinion_str = '%s_%s_%s' % (aspect_term_dict_to_str(opinion['aspect']),
                                        opinion['sentiment'],
                                        aspect_term_dict_to_str(opinion['opinion']))
            opinions.append(opinion_str)
        result[sentence] = opinions
...
for i in range(run_num):
    ...
    triplets_pred = get_sentence_and_triplets_pred(triplets_pred)
