...
parser.add_argument('--target_run', default=0, type=int)
    ...
    for sentence in sentences_true.keys():
        ...
        for e in triplets_true:
            if e in triplets_pred:
                tp += 1
        if triplets_true != triplets_pred:
            print('-' * 100)
            print('sentence: %s' % ' '.join(['%s-%d' % (word, k) for k, word in enumerate(sentence.split())]))
            print('true: %s' % str(triplets_true))
            print('pred: %s' % str(triplets_pred))
