...
from nlp_tasks.absa.mining_opinions.tosc import tosc_data_reader
from nlp_tasks.absa.mining_opinions.tosc import pytorch_models
...
class SpanBasedModelForAtsa(ModelTrainTemplate.ModelTrainTemplate):
    ...
    def _get_data_reader(self):
        if self.configuration['consider_target']:
            ...
            reader = tosc_data_reader.TextAspectInSentimentOutConsideringTarget(
            ...
            token_indexer = SingleIdTokenIndexer(namespace="tokens")
            position_indexer = SingleIdTokenIndexer(namespace='position')
            reader = tosc_data_reader.TextAspectInSentimentOut(
                self.distinct_polarities,
                tokenizer=self._get_word_segmenter(),
                token_indexers={"tokens": token_indexer},
                position_indexers={'position': position_indexer},
    ...
    def _load_data(self):
        ...
        if os.path.exists(data_filepath):
            ...
            train_dev_test_data, distinct_polarities = self.dataset.generate_tosc_data()
    ...
    def remove_backup_index(self, term: dict):
        """

        :param term:
        :return:
        """
        term['start'] = term['start_backup']
        term['end'] = term['end_backup']
        term.pop('start_backup')
        term.pop('end_backup')
...
class SpanBasedBertModelForTOSC(SpanBasedModelForAtsa):
    """
    2019-acl-Open-Domain Targeted Sentiment Analysisvia Span-Based Extraction and Classification
    """
    ...
    def _find_model_function_pure(self):
        return pytorch_models.SpanBasedBertModelForTOSC
